...
def extractor_opts(parser):
    ...
    group.add("-dict_file", required=True, help="Dictionary file")
    group.add("-vec_size", type=int, default=512, help=".")
    group.add("-type", default="random", choices=["random", "bert"], help=".")
    group.add("-seed", type=int, default=0, help=".")
    group.add("-vec_len", type=float, default=0.1, help=".")
...
if __name__ == "__main__":
    ...
    enc_file_name = (
        "random_encoder_emb.txt" if opt.type == "random" else "bert_encoder_emb.txt"
    )
    dec_file_name = (
        "random_decoder_emb.txt" if opt.type == "random" else "bert_decoder_emb.txt"
    )

    np.random.seed(opt.seed)
    vecs = np.random.uniform(
        -opt.vec_len, opt.vec_len, size=(len(enc_vocab), opt.vec_size)
    )
    ...
    vecs = np.random.uniform(
        -opt.vec_len, opt.vec_len, size=(len(dec_vocab), opt.vec_size)
    )
