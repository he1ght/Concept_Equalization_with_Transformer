...
def check_existing_pt_files(opt, corpus_type, ids, existing_fields):
    ...
    for maybe_id in ids:
        ...
        if glob.glob(pattern):
            if opt.overwrite:
                maybe_overwrite = (
                    "will be overwritten because " "`-overwrite` option is set."
                )
            else:
                maybe_overwrite = (
                    "won't be overwritten, pass the "
                    "`-overwrite` option if you want to."
                )
            logger.warning(
                "Shards for corpus {} already exist, {}".format(
        ...
        for ex in dataset.examples:
            for name, field in fields.items():
                ...
                for (sub_n, sub_f), fd in zip(f_iter, all_data):
                    ...
                    if (
                        hasattr(sub_f, "sequential")
                        and sub_f.sequential
                        and not has_vocab
                    ):
                        val = fd
        ...
        if opt.src_vocab != "":
            try:
                ...
                src_vocab, src_vocab_size = _load_vocab(
                    opt.src_vocab, "src", counters, opt.src_words_min_frequency
                )
        if opt.tgt_vocab != "":
            tgt_vocab, tgt_vocab_size = _load_vocab(
                opt.tgt_vocab, "tgt", counters, opt.tgt_words_min_frequency
            )
        ...
        for src, tgt, maybe_id, maybe_align in zip(srcs, tgts, ids, aligns):
            if maybe_id in existing_shards:
                if opt.overwrite:
                    ...
                    logger.warning(
                        "Ignore corpus {} because " "shards already exist".format(
                            maybe_id
                        )
                    )
                    continue
...
def preprocess(opt):
    ...
    for src, tgt in zip(opt.train_src, opt.train_tgt):
        src_nfeats += count_features(src) if opt.data_type == "text" else 0
