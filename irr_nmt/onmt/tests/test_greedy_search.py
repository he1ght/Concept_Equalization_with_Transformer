...
class TestGreedySearch(unittest.TestCase):
    ...
    def test_doesnt_predict_eos_if_shorter_than_min_len(self):
        ...
        for batch_sz in [1, 3]:
            ...
            samp = GreedySearch(
                0, 1, 2, batch_sz, min_length, False, set(), False, 30, 1.0, 1
            )
    ...
    def test_returns_correct_scores_deterministic(self):
        for batch_sz in [1, 13]:
            for temp in [1.0, 3.0]:
                ...
                lengths = torch.randint(0, 30, (batch_sz,))
                samp = GreedySearch(
                    0, 1, 2, batch_sz, 0, False, set(), False, 30, temp, 1
                )
                samp.initialize(torch.zeros(1), lengths)
                # initial step
                i = 0
                ...
                self.assertEqual(samp.scores[0], [valid_score_dist_1[0] / temp])
                if batch_sz == 1:
                ...
                samp.update_finished()
                self.assertEqual(samp.scores[8], [valid_score_dist_2[0] / temp])
    ...
    def test_returns_correct_scores_non_deterministic(self):
        for batch_sz in [1, 13]:
            for temp in [1.0, 3.0]:
                ...
                lengths = torch.randint(0, 30, (batch_sz,))
                samp = GreedySearch(
                    0, 1, 2, batch_sz, 0, False, set(), False, 30, temp, 2
                )
                samp.initialize(torch.zeros(1), lengths)
                # initial step
                i = 0
