...
class TestModel(unittest.TestCase):
    ...
    def get_batch_audio(
        self, tgt_l=7, bsize=3, sample_rate=5500, window_size=0.03, t=37
    ):
    ...
    def imagemodel_forward(self, opt, tgt_l=2, bsize=1, h=15, w=17):
        ...
        enc = ImageEncoder(opt.enc_layers, opt.brnn, opt.enc_rnn_size, opt.dropout)
        ...
        test_src, test_tgt, test_length = self.get_batch_image(
            h=h, w=w, bsize=bsize, tgt_l=tgt_l
        )
        outputs, attn = model(test_src, test_tgt, test_length)
