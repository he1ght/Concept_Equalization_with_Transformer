...
class ArgumentParser(cfargparse.ArgumentParser):
    ...
    def validate_model_opts(cls, model_opt):
        ...
        assert (
            model_opt.model_type == "audio" or same_size
    ...
    def validate_train_opts(cls, opt):
        ...
        if (
            opt.world_size == len(opt.gpu_ranks)
            and min(opt.gpu_ranks) > 0
            and opt.world_size != 1
        ):
            raise AssertionError(
                "-gpu_ranks should have master(=0) rank "
                "unless -world_size is greater than len(gpu_ranks)."
            )
        assert len(opt.data_ids) == len(
            opt.data_weights
    ...
    def validate_preprocess_args(cls, opt):
        ...
        if len(opt.train_align) == 1 and opt.train_align[0] is None:
            ...
            assert len(opt.train_align) == len(
                opt.train_src
        ...
        assert not opt.valid_align or os.path.isfile(
            opt.valid_align
        ), "Please check path of your valid alignment file!"

        assert not opt.valid_src or os.path.isfile(
            opt.valid_src
        ), "Please check path of your valid src file!"
        assert not opt.valid_tgt or os.path.isfile(
            opt.valid_tgt
        ), "Please check path of your valid tgt file!"

        assert not opt.src_vocab or os.path.isfile(
            opt.src_vocab
        ), "Please check path of your src vocab!"
        assert not opt.tgt_vocab or os.path.isfile(
            opt.tgt_vocab
