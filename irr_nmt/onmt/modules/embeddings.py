...
class BERTEmbedding(nn.Module):
    def __init__(self, word_padding_idx, fix_word_vecs=False):
        model = BertModel.from_pretrained("bert-base-multilingual-cased")
        model.eval()
        self.word_padding_idx = word_padding_idx
        bert_emb = model.embeddings
        self.word_vec_size = bert_emb.word_embeddings.embedding_dim
        self.embedding_size = bert_emb.word_embeddings.embedding_dim
    ...
    def forward(self, source, step=None):
        source = self.make_embedding(source.squeeze(-1).t())
