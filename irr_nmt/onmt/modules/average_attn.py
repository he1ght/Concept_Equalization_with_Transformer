...
class AverageAttention(nn.Module):
    ...
    def cumulative_average_mask(self, batch_size, inputs_len, device):
        ...
        triangle = torch.tril(
            torch.ones(inputs_len, inputs_len, dtype=torch.float, device=device)
        )
        weights = torch.ones(
            1, inputs_len, dtype=torch.float, device=device
    ...
    def forward(self, inputs, mask=None, layer_cache=None, step=None):
        ...
        average_outputs = self.cumulative_average(
            inputs,
            self.cumulative_average_mask(batch_size, inputs_len, inputs.device)
